...
免费的硬盘保护功能是个亮点（然而并没有开启；在大多数学校的使用环境下也没什么用，反而徒增麻烦），还有弹窗拦截功能。不过笔者向来提倡从源头解决问题，而不是靠拦截。如果需要此方面的帮助，请访问 [故障排除/清除系统内广告](../../troubleshooting/clean-ads.md)。

!!!important
	请不要在家用电脑上安装此软件。
	如果你 ~~手欠~~ 已经安装上了，可以遵循以下步骤卸载：  
	1. 打开 `C:\Program Files (x86)\Seewo\SeewoService` （对于 32 位系统则为 `C:\Program Files\Seewo\SeewoService`），运行目录下的 Uninstall.exe 程序。  
	2. 点按“确认”，输入默认密码 `000000` （六个0）开始卸载。（学校的电脑就别瞎折腾了，我没记错的话管理员密码已经被改掉了）
	3. 卸载完成后会弹出提示要求重启，如果没有，则手动重启系统。